...
const TrailingImage = (): JSX.Element => {
  ...
  const handlePrevImage = () => {
    setSelectedImage((prev) => {
      ...
      if (prev === 0) return Landscape.length - 1;
      return prev - 1;
    });
  };
  ...
  const handleNextImage = () => {
    setSelectedImage((prev) => {
      if (prev === null) return null;
      if (prev === Landscape.length - 1) return 0;
      return prev + 1;
    });
  };
  ...
  return (
    <>
      <div className="block">
        {Landscape.map(({ src, alt, title, index }) => (
          <img
            key={index}
            src={src}
            alt={alt}
      ...
      {selectedImage !== null && (
        ...
        >
          ...
          <img
            src={Landscape[selectedImage]?.src}
            alt={Landscape[selectedImage]?.alt}
